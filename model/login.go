...
func GetLoginByToken(token string) (*Login, error) {
	var login Login
	err := Database.QueryRow("select id, user_id, token, login_at, login_ip from im_login where token=?", token).Scan(&login.Id, &login.UserId, &login.Token, &login.LoginAt, &login.LoginIp)
	if err != nil {
