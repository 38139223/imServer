...
func  (user User)Salt() string {
	var userSalt string
/*	num,_:= CheckAccount(user.Account)
	if num < 1 {
		userSalt = strconv.Itoa(int(time.Now().Unix()))
	} 减少一次查询*/
		err := Database.QueryRow("select password from im_user where account=?",user.Account).Scan(&userSalt)
		if err !=nil {
...
func  LoginUser(account string, password string) (*User, error) {
	...
	pw := EncryptPassword(password,user.Salt())
	rows, err := Database.Query("select id, nick, status, sign, avatar, create_at, update_at from im_user where account=? and password=? ", account, pw)
	if err != nil {
...
func SaveUser(account string, password string, nick string, avatar string) (*string, error) {
	user := User{Account:account,Password:password,Nick:nick,Avatar:avatar}
	pw := EncryptPassword(password,user.Salt())
	...
	_, err = insStmt.Exec(uid, account, pw, nick, avatar, now, now)
	if err != nil {
